...
public class main{
	public static void Main(){
		...
		Func<double,double> gw_diff = x => (1-2*x*x)*Exp(-x*x);
		Func<double,double> gw_int = x => -Exp(-x*x)/2;
		Func<double,double> g = x => Cos(5*x-1)*Exp(-x*x); //test function


		int n = 5; //Hidden neurons
		ann network = new ann(n, gw, gw_diff, gw_int);//constructing the network
		...
		for(double z=a; z<b; z+=1.0/64){
			WriteLine($"{z} {network.response(z)} {network.response_diff(z)} {network.response_int(z)}");
		}
	}//Main
