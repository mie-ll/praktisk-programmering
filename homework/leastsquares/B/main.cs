...
public class main{
	...
	public static void Main(){
		...
		using(var outfile = new System.IO.StreamWriter("fitlog.txt")){
			outfile.WriteLine("\n");
			outfile.WriteLine($"Fit parameters: a = {cs[0]}, lambda = {cs[1]}+/-{Math.Sqrt(S[1,1])}");
			outfile.WriteLine($"The half-life is found by Ln(2)/lambda and is {Log(2)/(-cs[1])} +/- {Math.Sqrt(S[1,1])*Math.Log(2)/(-cs[1])} days");
